    ...
    while i != number_of_frames:
        ...
        if rec.AcceptWaveform(b''.join(frame)):
            ...
            if previous_sentence!=sentence:
                ...
                sid = np.arange(start_frame, end_frame+1)
                
                if word =="galactic" or word =="battery" or word =="oxygen" or word =="temperature":
                    lbl = [0 for _ in range(len(sid))]
                    label_samples(sid, lbl)
                else:
                    lbl = [1 for _ in range(len(sid))]
                    label_samples(sid, lbl)    
